...
function Admin() {
  ...
  const data = useQuery({
    ...
    queryFn: async () => {
      const [profiles, properties, withdrawals, leaders] = await Promise.all([
        ...
        supabase.from("properties").select("id, title, status, rent_ugx, landlord_id, created_at, locations(name, full_path), areas(name, cities(name))").order("created_at", { ascending: false }),
        ...
        supabase.rpc("get_referral_leaders"),
      ]);
      const error = profiles.error ?? properties.error ?? withdrawals.error ?? leaders.error;
      if (error) throw error;
      return {
        profiles: profiles.data ?? [],
        properties: properties.data ?? [],
        withdrawals: withdrawals.data ?? [],
        referralSummary: leaders.data ?? [],
      };
    },
  });
